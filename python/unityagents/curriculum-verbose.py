            ...
            for key, value in progress.items():
                ...
                if self.lesson_number < self.max_lesson_number:
                    if(self.threshold_operator == 'or'):
                        ...
                        logger.info("\n threshold check OR - {0}:{1} , compared to: {2}".format(key,value,self.data[key][self.lesson_number]))
                    else:
                        change_lesson = change_lesson and (value > self.data[key][self.lesson_number])
                        logger.info("\n threshold check AND - {0}:{1} , compared to: {2}".format(key,value,self.data[key][self.lesson_number]))
                    logger.info("\n threshold result: {0}, lesson length( {1} ) > min length ( {2} )".format(change_lesson, self.lesson_length, self.data['min_lesson_length']))    
        else:
            if self.data["signal_smoothing"]:
