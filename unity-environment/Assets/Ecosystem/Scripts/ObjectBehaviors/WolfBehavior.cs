    ...
    public class WolfBehavior : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            if (Agent != null)
            {
                ...
                //float newDist = Vector3.Distance(FindClosestTargetPointsByName("Sheep(Clone)", 1, MaxSight)[0], transform.position);
        ...
        {
            Agent.Dead = true;
            this.Spawner.WolfDeath(this.gameObject);
        ...
        public List<float> GetObservations()
        {
            ...
            Vector3[] rewards = FindClosestObjectPosByName("PLANT(Clone)", 1, MaxSight);

            GameObject[] sheep = FindClosestObjectByName("Sheep(Clone)", 2, MaxSight);

            GameObject[] wolf = FindClosestObjectByName("Wolf(Clone)", 1, MaxSight);
            //Temp variables for assigning distance values
            float tmpX = 0, tmpZ = 0;


            //Adding eaten plant data
            state.Add(this.SheepEaten * 0.01f);
            ...
            //Adding position of the other wolf
            //and the closest 
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (Agent != null && other.gameObject.tag == "Sheep")
            {
                Agent.AddReward(0.7f);
        ...
        public bool InSight(GameObject g)
        {
